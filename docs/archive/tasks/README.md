# Task notes

These files are an archive of the original prototype workflow. Their `src/`,
`content/`, and `extension/` paths describe the pre-monorepo layout; current
paths are documented in the root README and the live documents in `docs/`.

The authoritative current documents are:

- [`PRODUCT.md`](../../PRODUCT.md): business rules and product decisions.
- [`DESIGN.md`](../../DESIGN.md): visual and interaction rules.
- [`PLAN.md`](../../PLAN.md): implementation phases and acceptance criteria.
- [`ROADMAP.md`](../../ROADMAP.md): shipped work, gaps, and next steps.

The individual `T*.md` files are historical task specifications from the first
site prototype. They may explain existing code, but they must not be
used to reopen decisions that `PRODUCT.md` has locked, especially per-repository
companions, authentication-first onboarding, retroactive XP, or Pokémon assets
as a commercial product.

Use a new task file only when a phase in `PLAN.md` needs a concrete execution
brief. Name it after the current phase and link it from `ROADMAP.md`.
